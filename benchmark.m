...
C_1_plus.func = @(k) 9./((sqrt(10)+1).*(k+I))-6./((sqrt(10)+2).*(k+2.*I))+exp(I.*k.*phi)./...
    ((2.*sqrt(10)+2).*(k+I))-exp(I.*k.*phi)./((2.*sqrt(10)+6).*(k+3.*I))-(exp(I.*k.*phi)-exp(-sqrt(10).*phi))./...
...
C_1_minus.func = @(x) ((sqrt(10)-1)./(x-I.*sqrt(10))-1./(x-2.*I))./(sqrt(10)-2)-exp(-sqrt(10).*phi)./((sqrt(10)+3).*(sqrt(10)+1).*(x-I.*sqrt(10)));

C_1_minus.array = C_1_minus.func(X);


g2_plus = @(k) gamma.*exp(-sqrt(10).*phi).*sqrt(10)./((k+I.*sqrt(10)).*(20.*sqrt(10)+40))+(1./8).*(gamma^2-1)./(k+3.*I)+(1./4).*...
    (-gamma^2+81)./((k+I).*9)-(1./20).*gamma^2.*sqrt(10)./((9.*(sqrt(10)-3)).*(k+I.*sqrt(10)));


g2_minus = @(x) -gamma.*exp(-I.*x.*phi).*(1./(6.*x-12.*I)-sqrt(10)./((x-I.*sqrt(10)).*(20.*sqrt(10)-40)))+gamma.*(exp(-I.*x.*phi)-...
    exp(-sqrt(10).*phi)).*sqrt(10)./((x+I.*sqrt(10)).*(20.*sqrt(10)+40))+1./(x-3.*I)+(1./8).*(gamma^2-81)./...
    (9.*x-9.*I)-(1./20).*gamma^2.*sqrt(10)./((9.*sqrt(10)+27).*(x-I.*sqrt(10)));
